...
function generateMultipleChoiceGroupQuestion(params, domRoot) {
  ...
  $('#submit_' + toplevel_tag).click(function() {
    ...
    if (numCorrect == questionsList.length) {
      ...
      $('#output_' + toplevel_tag).val("All your answers are correct! " + HTMLEncode(allCorrectOutput));
    }
    else {
      $('#output_' + toplevel_tag).val("You got " + numCorrect + " out of " + questionsList.length + " questions correct. " + HTMLEncode(someIncorrectOutput));
    }
  });
}
...
function generateFreetextQuestion(params, domRoot) {
  ...
  if (showAnswerOutput) {
    domRoot.append('<p/><a class="gcb-button gcb-button-primary" id="skip_and_show_' + tag + '">' +  HTMLEncode(showAnswerPrompt) + '</a>');
  }
